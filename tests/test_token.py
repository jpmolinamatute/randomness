...
def test_load_tokens_missing_file() -> None:
    ...
    with (
        ...
        pytest.raises(TokenError, match="Token file not found"),
    ):
        token.load_tokens()
...
def test_load_tokens_json_decode_error() -> None:
    ...
    with (
        ...
        pytest.raises(TokenError, match="Failed reading token file"),
    ):
        token.load_tokens()
...
def test_load_tokens_missing_keys() -> None:
    ...
    with (
        patch("pathlib.Path.exists", return_value=True),
        patch("pathlib.Path.open", m),
        pytest.raises(TokenError, match="Token file missing key"),
    ):
        token.load_tokens()
...
def test_load_tokens_success() -> None:
    ...
    token = Token()
    valid_data = (
        f'{{"access_token": "acc", "refresh_token": "ref", "token_expires_at": {MOCK_EXPIRES_AT}}}'
    )
...
def test_store_tokens_permission_error() -> None:
    ...
    with (
        patch("pathlib.Path.open", m),
        pytest.raises(TokenError, match="Failed storing token file"),
    ):
        token.store_tokens()
